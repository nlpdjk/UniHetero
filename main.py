    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(data_loader):
                ...
                outputs = torch.argmax(outputs, -1)
                ent_c, ent_p, ent_r, _ = decode.decode(outputs.cpu().numpy(), entity_text, length.cpu().numpy())
        ...
        p, r, f1, _ = precision_recall_fscore_support(label_result.numpy(),
                                                      pred_result.numpy(),
                                                      average="macro")
        e_f1, e_p, e_r = decode.cal_f1(total_ent_c, total_ent_p, total_ent_r)
    ...
    def predict(self, epoch, data_loader, data):
        ...
        with torch.no_grad():
            for data_batch in data_loader:
                ...
                outputs = torch.argmax(outputs, -1)
                ent_c, ent_p, ent_r, decode_entities = decode.decode(outputs.cpu().numpy(), entity_text, length.cpu().numpy())
...
if __name__ == '__main__':
    ...
    logger = decode.get_logger(config.dataset)
    logger.info(config)
